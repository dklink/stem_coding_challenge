...
if __name__ == "__main__":
    ...
    print(f"\tStatus code: {user_response.status_code}; Response: {user_response.json()}")
    print("Put that api key somewhere safe, you'll need it to authenticate the other endpoints!")
    user1 = user_response.json()
    print()

    print(f"Now, we'll add some mood captures for user {user1['user_id']} using POST /mood-captures")
    r1 = requests.post(
    ...
    print(f"Now, let's get the distribution of user {user1['user_id']}'s moods using GET /mood-captures/frequency-distribution")
    r5 = requests.get(
    ...
    print("Oops, we forgot to include the user's api key in the headers!  Let's try again.")
    r6 = requests.get(
        base_url + f"/mood-captures/frequency-distribution?user_id={user1['user_id']}",
        headers={"x-api-key": user1["api_key"]}
    )
